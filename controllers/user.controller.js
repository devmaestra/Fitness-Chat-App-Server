...
router.get("/matches", validateSession, async (req, res) => {
  ...
  try {
    ...
    let getMatchByZip = await User.find({
      locationZip: { $in: localRadiusZips },
      active: true,
    }); // Use ARRAY to find by Zip Code if Active: true.
    ...
    console.log(
      `Logged in as ${userName}. ID: ${userId} Here are your MATCHES:`
    );
